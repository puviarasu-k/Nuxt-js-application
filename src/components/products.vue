...
<template>
    <main class="flex w-full h-full` flex-col gap-4 p-4">
        <dialog id="favDialog" open="false"
            class="absolute mx-auto border-2 w-1/2 h-2/3 flex justify-center mt-20 border-orange-700/50 rounded-xl shadow-2xl"
            v-if="this.productadd || this.editclick">
            <form method="dialog" class="flex flex-col h-full w-1/2 items-center justify-evenly" @submit="actionclick">
                <h1
                    class="bg-orange-400/50 px-6 py-2 rounded-full font-semibold text-xl hover:bg-orange-500/50 select-none">
                    Add Product</h1>
                <div class="flex justify-between w-full items-center">
                    <label class="text-lg">Product Name: </label>
                    <input type="text" required v-model="productName"
                        class="pl-2 w-60 h-10 border-solid outline-orange-500 border-[1px] border-orange-500 rounded-md"
                        placeholder="Enter Product Name">
                </div>
                <div class="flex justify-between w-full items-center">
                    <label class="text-lg">Product Category:</label>
                    <input type="text" required v-model="category"
                        class="pl-2 w-60 h-10 border-solid outline-orange-500 border-[1px] border-orange-500 rounded-md"
                        placeholder="Enter Product Category">
                </div>
                <div class="flex justify-between w-full items-center">
                    <label class="text-lg">Product Amount:</label>
                ...
                <div>
                    <button value="cancel"
                        class="rounded-lg active:scale-105 font-semibold ml-5 px-6 py-2 bg-orange-500 hover:bg-orange-600"
                        @click.prevent="this.productadd ? this.productadd = !this.productadd : this.editclick = !this.editclick">Cancel</button>
                    <button id="confirmBtn" value="default" type="submit"
                        class="rounded-lg active:scale-105 font-semibold ml-5 px-6 py-2 bg-orange-500 hover:bg-orange-600 ">Submit</button>
                </div>
            </form>
        </dialog>
        <button @click="this.productadd = !this.productadd"
            class="self-end bg-orange-500 px-6 rounded-lg font-semibold py-2 hover:bg-orange-600 active:scale-105 select-none">Add
            Product</button>
        <table class="table-auto shadow-2xl" :class="`${this.productadd ? `hidden` : ``}`">
            <thead>
                <tr class="border-2">
